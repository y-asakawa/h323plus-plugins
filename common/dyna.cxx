...
AVCodec *FFMPEGLibrary::AvcodecFindEncoder(enum FF_CodecID id)
{
  ...
  WITH_ALIGNED_STACK({
    ...
    return NULL;
#else
    const AVCodec *res = avcodec_find_encoder(id);
    return (AVCodec *)res;
#endif
#endif
}
...
#else
    const AVCodec *res = avcodec_find_decoder(id);
    return (AVCodec *)res;
#endif
}
...
int FFMPEGLibrary::AvcodecEncodeVideo(AVCodecContext *ctx, BYTE *buf, int buf_size, const AVFrame *pict)
{
     ...
     int gotFrame = 0;
      ...
      err = avcodec_send_frame(ctx, pict);
      if (err < 0)
        return 0;
      ...
      const int packetSize = pkt.size;
      memmove(buf, pkt.data, packetSize);
      TRACE_UP(4, _codecString << "\tDYNA\tEncoded " << buf_size << " bytes of YUV420P data into " << packetSize << " bytes");
      av_packet_unref(&pkt);
      return packetSize;
#endif
#endif
...
int res=0;
     ...
     pkt.size = buf_size;
      ...
      WITH_ALIGNED_STACK({
        ...
        res = avcodec_receive_frame(ctx, pict);
        if (res == 0) {
          *got_picture_ptr = 1;
          return buf_size;
        }
...
void FFMPEGLibrary::AvSetDimensions(AVCodecContext *s, int width, int height)
{
  ...
  WITH_ALIGNED_STACK({
    ...
    s->width = width;
    s->height = height;
    avcodec_align_dimensions(s, &s->width, &s->height);
#endif
#endif
}
