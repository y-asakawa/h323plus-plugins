...
ifndef OPENH323DIR
OPENH323DIR	= $(abspath $(dir $(lastword $(MAKEFILE_LIST))))
endif
PTLIBDIR	?= $(abspath $(OPENH323DIR)/../ptlib)
STDCCFLAGS	+= 
LDFLAGS		+= 
LDLIBS		+= 
ENDLDLIBS	:=  $(ENDLDLIBS)
...
# Sanitize stale Homebrew OpenSSL paths and noisy Darwin ld flags from ptlib
LDFLAGS := $(filter-out -multiply_defined suppress,$(LDFLAGS))
ENDLDLIBS := $(filter-out -lresolv,$(ENDLDLIBS)) -lresolv
HOMEBREW_OPENSSL_PREFIX ?= $(shell command -v brew >/dev/null 2>&1 && brew --prefix openssl@3 2>/dev/null)
ifneq ($(strip $(HOMEBREW_OPENSSL_PREFIX)),)
ENDLDLIBS := $(filter-out -L$(HOMEBREW_OPENSSL_PREFIX)/lib,$(ENDLDLIBS))
STDCCFLAGS := $(filter-out -I$(HOMEBREW_OPENSSL_PREFIX)/include,$(STDCCFLAGS))
STDCXXFLAGS := $(filter-out -I$(HOMEBREW_OPENSSL_PREFIX)/include,$(STDCXXFLAGS))
endif
