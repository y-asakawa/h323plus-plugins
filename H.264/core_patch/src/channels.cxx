...
static DWORD GetAudioRtpTimestampStep(RTP_DataFrame::PayloadTypes payloadType, DWORD codecFrameRate)
{
  // RFC 3551 requires G.722 RTP timestamping at 8kHz clock rate.
  if (payloadType == RTP_DataFrame::G722 && codecFrameRate > 1)
    return codecFrameRate/2;
...
void H323_RTPChannel::Transmit()
{
  ...
  // Keep G.722 packetization at 20 ms (1 ms frame x 20) for interoperability.
  if (isAudio && rtpPayloadType == RTP_DataFrame::G722) {
    framesInPacket = 20;
  } else if (framesInPacket > 8) {
    framesInPacket = 1;  // TODO: Resolve issue with G.711 20ms
  }
  ...
  while (codec->Read(frame.GetPayloadPtr()+frameOffset, length, frame)) {
    ...
    if(isAudio)
    {
        rtpTimestamp += GetAudioRtpTimestampStep(rtpPayloadType, codec->GetFrameRate());
    }
    else
    {
       if(frame.GetMarker()) {
...
void H323_RTPChannel::Receive()
{
  ...
  while (ReadFrame(rtpTimestamp, frame)) {
    ...
    if (payloadSize == 0) {
      ...
      rtpTimestamp += GetAudioRtpTimestampStep(rtpPayloadType, codecFrameRate);
    } else {
      silenceStartTick = PTimer::Tick().GetMilliSeconds();
