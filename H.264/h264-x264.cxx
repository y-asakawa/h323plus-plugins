 ...
 *                 Y. Asakawa (2025) - macOS/ARM64 compatibility fixes
 *
 *
 */
...
#ifdef _STATIC_LINK
  /* When using static x264 linking but loading as dynamic plugin,
   * we define _STATIC_LINK for enc-ctx.cxx compilation but NOT OPAL_STATIC_CODEC
   * so that OpalCodecPlugin_GetCodecs is exported for dynamic loading */
  #ifdef _WIN32
...
/* For non-Windows with _STATIC_LINK but still dynamic plugin loading */
#if defined(_STATIC_LINK) && !defined(_WIN32) && !defined(OPAL_STATIC_CODEC)
  #define PLUGIN_CODEC_DLL_EXPORTS  1
#endif
...
H264DecoderContext::~H264DecoderContext()
{
  ...
  {
    // Use av_frame_free() for AVFrame BEFORE freeing context
    // (frame may reference context internals)
    if (_outputFrame != NULL) {
    ...
    if (_context != NULL)
    {
      // AvcodecClose now just marks codec as closed (FFmpeg 58+)
      // We need to call avcodec_free_context ourselves
      ...
      // In static-link builds, AvcodecClose() already frees the context
      // for FFmpeg 58+, so don't double-free here.
#if defined(_STATIC_LINK) && (LIBAVCODEC_VERSION_MAJOR >= 58)
      _context = NULL;
#else
...
#if LIBAVCODEC_VERSION_MAJOR >= 58
  if (_outputFrame->pict_type == AV_PICTURE_TYPE_I)
#else
